...
#pragma warning disable 0649
		...
		[SerializeField]
		[HideInInspector]
		private float minimumHeight = 50f;
        ...
        [SerializeField]
        private TMP_InputField commandInputFieldTMP;
        ...
        [SerializeField]
        private TextMeshProUGUI infoEntryCountTextTMP;
        [SerializeField]
        private TextMeshProUGUI warningEntryCountTextTMP;
        [SerializeField]
        private TextMeshProUGUI errorEntryCountTextTMP;
...
#pragma warning restore 0649
        ...
        public TMP_InputField CommandInputFieldTMP { get => commandInputFieldTMP; set => commandInputFieldTMP = value; }
		...
		private void OnEnable()
		{
			...
			CommandInputFieldTMP.onValidateInput -= OnValidateCommand;
			CommandInputFieldTMP.onValidateInput += OnValidateCommand;
		...
		private void LateUpdate()
		{
			...
			if( isLogWindowVisible && CommandInputFieldTMP.isFocused )
			{
				if( Input.GetKeyDown( KeyCode.UpArrow ) )
				{
					...
					{
						CommandInputFieldTMP.text = commandHistory[commandHistoryIndex];
						CommandInputFieldTMP.caretPosition = CommandInputFieldTMP.text.Length;
					}
				}
			...
			if( addedChar == '\t' ) // Autocomplete attempt
			{
				if( !string.IsNullOrEmpty( text ) )
				{
					...
					if( !string.IsNullOrEmpty( autoCompletedCommand ) )
						CommandInputFieldTMP.text = autoCompletedCommand;
				}
		...
		private void ReceivedLog( string logString, string stackTrace, LogType logType )
		{
			...
			{
				infoEntryCount++;
				infoEntryCountTextTMP.text = infoEntryCount.ToString();
			...
			{
				warningEntryCount++;
				warningEntryCountTextTMP.text = warningEntryCount.ToString();
			...
			{
				errorEntryCount++;
				errorEntryCountTextTMP.text = errorEntryCount.ToString();
		...
		public void ClearButtonPressed()
		{
			...
			infoEntryCountTextTMP.text = "0";
			warningEntryCountTextTMP.text = "0";
			errorEntryCountTextTMP.text = "0";
