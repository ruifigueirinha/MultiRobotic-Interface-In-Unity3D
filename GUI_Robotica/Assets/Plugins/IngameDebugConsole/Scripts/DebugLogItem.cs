...
#pragma warning disable 0649
        ...
        [SerializeField]
        private TextMeshProUGUI logCountTextTMP;
#pragma warning restore 0649
		...
		public void SetContent( DebugLogEntry logEntry, int entryIndex, bool isExpanded )
		{
			...
			if( isExpanded )
			{
                ...
                //logTextTMP.enableWordWrapping = true;
			...
			else
			{
                ...
                //logTextTMP.overflowMode = TMPro.TextOverflowModes.Overflow;
                size.y = manager.ItemHeight;
			}
			transformComponent.sizeDelta = size;

			logTextTMP.text = isExpanded ? logEntry.ToString() : logEntry.logString;
		...
		{
			logCountTextTMP.text = logEntry.count.ToString();
		...
		{
			string text = logTextTMP.text;
			//HorizontalWrapMode wrapMode = logText.horizontalOverflow;
            bool wrapModeTMP = logTextTMP.enableWordWrapping;


			logTextTMP.text = content;
			//logText.horizontalOverflow = HorizontalWrapMode.Wrap;

			float result = logTextTMP.preferredHeight;

			logTextTMP.text = text;

            //logText.horizontalOverflow = wrapMode;
            //logTextTMP.enableWordWrapping = wrapModeTMP;
